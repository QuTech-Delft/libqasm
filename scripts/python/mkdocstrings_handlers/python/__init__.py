...
class PythonHandler(BaseHandler):
    def __init__(self, **kwargs: Any) -> None:
        ...
        include_dir = os.path.join(top_dir, 'python', 'module', 'cqasm', 'v3x')
        self._ns2doxyxml = {}
    ...
    def collect_compound(self) -> Definition:
        ...
        path = os.path.join(self._doxyxml_dir, 'classcqasm_1_1v3x_1_1Analyzer.xml')
        with open(path) as f:
            xml = ElementTree.parse(f)
            node = xml.find('compounddef')
            d = Definition('cqasm::v3x::Analyzer', node=node)

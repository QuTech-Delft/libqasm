...
class EmscriptenHandler(BaseHandler):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(handler='emscripten', **kwargs)
        ...
        include_dir = os.path.join(top_dir, 'emscripten')
        self._ns2doxyxml = {}
        build_dir = os.path.join(top_dir, 'build', 'Release', 'doc', 'ts')
        ...
        for h in headers:
            ...
            filename = filename.replace(".hpp", "_8hpp.xml")
    ...
    def collect_compound(self) -> Definition:
        ...
        path = os.path.join(self._doxyxml_dir, 'structEmscriptenWrapper.xml')
        with open(path) as f:
            xml = ElementTree.parse(f)
            node = xml.find('compounddef')
            d = Definition('EmscriptenWrapper', node=node)
...
def get_handler(
    ...
    **_config: Any
    ...
    """Return an instance of `EmscriptenHandler`.
    ...
    """
    return EmscriptenHandler(theme=theme, custom_templates=custom_templates)

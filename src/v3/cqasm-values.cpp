/** \file
 * Implementation for \ref include/v3/cqasm-values.hpp "v3/cqasm-values.hpp".
 */
...
Value promote(const Value &value, const types::Type &type) {
    ...
    if (type->as_real()) {
        if (auto const_int = value->as_const_int()) {
            retval = tree::make<values::ConstReal>(static_cast<double>(const_int->value));
        }
    }
    ...
    if (type->as_complex()) {
        if (auto const_int = value->as_const_int()) {
            retval = tree::make<values::ConstComplex>(static_cast<double>(const_int->value));
        } else if (auto const_real = value->as_const_real()) {
            retval = tree::make<values::ConstComplex>(std::complex<double>(const_real->value, 0.));
        }
    }

/** \file
 * Implementation for \ref include/v3/cqasm-analyzer.hpp "v3/cqasm-analyzer.hpp".
 */
...
public:
    ...
    /**
     * Adds an analyzed statement to the current block.
     */
     ...
     * current subcircuit (API 1.0/1.1) or block (API 3.0).
     */
     ...
     * subblock (3.0). Handles the requisite scoping, then defers to
     * analyze_statements().
     */
    ...
    /**
     * Analyzes the given bundle and, if valid, adds it to the current
     * scope/block using API version 3.0. If an error occurs, the message is
    ...
    /**
     * Analyzes the given cQASM 3.0 set instruction. If an error occurs, the
    ...
    /**
     * Analyzes the given cQASM 3.0 goto instruction. If an error occurs, the
     * message is added to the result error vector, and an empty Maybe is
     * returned.
     */
     ...
     * it to the current scope/block using API version 3.0. If an error occurs,
...
/**
 * Adds an analyzed statement to the current block (3.0).
 */
        ...
        try {
            if (auto bundle = stmt->as_bundle()) {
                ...
                if (result.root->version->items.compare("1.2") < 0) {
                    throw error::AnalysisError("structured control-flow is not supported (need version 3.0)");
                }
...
/**
 * Analyzes the given bundle and, if valid, adds it to the current
 * subcircuit using API version 3.0. If an error occurs, the message is
 * added to the result error vector, and nothing is added to the subcircuit.
 */

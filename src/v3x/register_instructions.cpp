...
void register_instructions(analyzer::Analyzer* analyzer) {
    ...
    for (const auto& [gate_name, pair_params_types_operands_types] : gate_map) {
        if (instruction_set.is_single_qubit_gate(gate_name)) {
            const auto& operand_types = pair_params_types_operands_types.second;
            const auto& single_qubit_unitary_gate_name =
                fmt::format("{}_{}", instruction_set.single_qubit_unitary_gate_composition_prefix, gate_name);
    ...
    for (const auto& [gate_name, pair_params_types_operands_types] : gate_map) {
        if (instruction_set.is_single_qubit_gate(gate_name)) {
            ...
            const auto& two_qubit_unitary_gate_name =
                fmt::format("{}_{}", instruction_set.two_qubit_unitary_gate_composition_prefix, gate_name);

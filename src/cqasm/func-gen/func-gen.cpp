...
/*
    register_function("operator+", "mm", [](const values::Values &v) {
        auto lhs = v[0]->as_const_real_matrix()->value;
        auto rhs = v[1]->as_const_real_matrix()->value;
        if (lhs.size_cols() != rhs.size_cols() || lhs.size_rows() != rhs.size_rows()) {
            throw error::AnalysisError(
                "matrix size mismatch: ("
                + std::to_string(lhs.size_cols())
                + ", " + std::to_string(lhs.size_rows())
                + ") vs (" + std::to_string(rhs.size_cols())
                + ", " + std::to_string(rhs.size_rows())
                + ")");
        }
        auto retval = primitives::RMatrix(lhs.size_cols(), lhs.size_rows());
        for (size_t row = 1; row <= lhs.size_rows(); row++) {
            for (size_t col = 1; col <= lhs.size_rows(); col++) {
                retval.at(row, col) = lhs.at(row, col) + rhs.at(row, col);
            }
        }
        return tree::make<values::ConstRealMatrix>(retval);
    });

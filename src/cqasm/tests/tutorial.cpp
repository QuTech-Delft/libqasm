...
TEST(tutorial, tutorial) {
    ...
    // (cqasm::tree::One<cqasm::semantic::Program> to be specific), or the
    // error messages printed to stderr and a cqasm::analysis::AnalysisError
    // exception (derived from std::runtime_error) on failure.
    auto result = cqasm::analyze("grover.cq");
    ...
    // code), and there's also cqasm::analyze_string():
    cqasm::analyze_string("version 1.0; qubits 1; h q[0]; measure q[0]; display");
    ...
    qubit_refs->index.add(cqasm::tree::make<cqasm::values::ConstInt>(6));
    ...
    // And add another one:
    qubit_refs->index.add(cqasm::tree::make<cqasm::values::ConstInt>(7));
    ...
    // to copy (in this case) cqasm::values::Node using its constructor, because
    ...
    //    through the cqasm::ast namespace);
    //
    //  - the semantic tree (defined in src/cqasm-semantic.tree, available
    //    through the cqasm::semantic namespace);
    //
    ...
    //    src/cqasm-types.tree, available through the cqasm::types namespace).
    //
...
R"(QubitRefs( # grover.cq:10:9..15
  index: [
    ...
    // of the type cqasm::parser::SourceLocation in this case:
    EXPECT_EQ(clone->get_annotation<cqasm::parser::SourceLocation>().filename, "grover.cq");

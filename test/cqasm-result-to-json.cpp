...
TEST(v1x_parser, errors) {
    auto input_file_path = fs::path{"res"}/"v1x"/"parsing"/"grammar"/"expression_recovery"/"input.cq";
    cqasm::v1x::analyzer::Analyzer analyzer{};
    auto semantic_ast_result = analyzer.analyze(input_file_path.generic_string());
    auto json_result = semantic_ast_result.to_json();
...
TEST(v1x_parser, ast) {
    ...
    auto json_result = ast_result.to_json();
    auto expected_json_result = std::string{
...
TEST(v1x_analyzer, errors) {
    auto input_file_path = fs::path{"res"}/"v1x"/"parsing"/"misc"/"wait_not_ok_1"/"input.cq";
    auto semantic_ast_result = cqasm::v1x::default_analyzer().analyze(input_file_path.generic_string());
    auto json_result = semantic_ast_result.to_json();
    auto expected_json_result = std::string{
...
TEST(v3x_parser, ast) {
    ...
    auto json_result = ast_result.to_json();
    auto expected_json_result = std::string{
        R"delim({"Program":{"version":{"Version":{"items":"3","source_location":"<unknown>:1:9..10"}},"statements":{"StatementList":{"items":[{"Variables":{"names":[{"Identifier":{"name":"b"}}],"typ":{"Identifier":{"name":"bit"}},"size":{"IntegerLiteral":{"value":"0"}},"annotations":"[]","source_location":"<unknown>:3:8..9"}}]}}}})delim"
    };
    EXPECT_EQ(json_result, expected_json_result);
}
TEST(v3x_analyzer, errors) {
    auto input_file_path = fs::path{"res"}/"v3x"/"parsing"/"bit_array_definition"/"bit_array_of_0_b"/"input.cq";
    auto semantic_ast_result = cqasm::v3x::default_analyzer().analyze(input_file_path.generic_string());
    auto json_result = semantic_ast_result.to_json();
    auto expected_json_result = std::string{

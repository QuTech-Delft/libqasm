import functools
import struct
import cqasm.v3.primitives
import cqasm.v3.values
import cqasm.v3.types
import cqasm.v3.error_model
import cqasm.v3.instruction
...
class Annotated(Node):
    ...
    def _serialize(self, id_map):
        ...
        for key, val in self._annot.items():
            cbor['{%s}' % key] = _py_to_cbor(cqasm.v3.primitives.serialize(key, val))
...
class AnnotationData(Node):
    ...
    def interface(self, val):
        if val is None:
            del self.interface
            return
        if not isinstance(val, cqasm.v3.primitives.Str):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('interface must be of type cqasm.v3.primitives.Str')
            val = cqasm.v3.primitives.Str(val)
        self._attr_interface = val

    @interface.deleter
    def interface(self):
        self._attr_interface = cqasm.v3.primitives.Str()
    ...
    def operation(self, val):
        ...
        if not isinstance(val, cqasm.v3.primitives.Str):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('operation must be of type cqasm.v3.primitives.Str')
            val = cqasm.v3.primitives.Str(val)
        self._attr_operation = val

    @operation.deleter
    def operation(self):
        self._attr_operation = cqasm.v3.primitives.Str()
    ...
    def operands(self, val):
        if val is None:
            del self.operands
            return
        if not isinstance(val, cqasm.v3.values.MultiNode):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('operands must be of type cqasm.v3.values.MultiNode')
            val = cqasm.v3.values.MultiNode(val)
        self._attr_operands = val

    @operands.deleter
    def operands(self):
        self._attr_operands = cqasm.v3.values.MultiNode()
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if hasattr(cqasm.v3.primitives.Str, 'deserialize_cbor'):
            f_interface = cqasm.v3.primitives.Str.deserialize_cbor(field)
        else:
            f_interface = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Str, field)
        ...
        if hasattr(cqasm.v3.primitives.Str, 'deserialize_cbor'):
            f_operation = cqasm.v3.primitives.Str.deserialize_cbor(field)
        else:
            f_operation = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Str, field)
        ...
        f_operands = cqasm.v3.values.MultiNode()
        for element in data:
            if element.get('@T') != '1':
                raise ValueError('unexpected edge type for Any/Many element')
            f_operands.append(cqasm.v3.values.Node._deserialize(element, seq_to_ob, links))
        ...
        for key, val in cbor.items():
            if not (key.startswith('{') and key.endswith('}')):
                continue
            key = key[1:-1]
            node[key] = cqasm.v3.primitives.deserialize(key, val)
    ...
    def _serialize(self, id_map):
        ...
        if hasattr(self._attr_interface, 'serialize_cbor'):
            ...
            cbor['interface'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Str, self._attr_interface)
        ...
        if hasattr(self._attr_operation, 'serialize_cbor'):
            ...
            cbor['operation'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Str, self._attr_operation)
...
class ErrorModel(Annotated):
    ...
    def model(self, val):
        if val is None:
            del self.model
            return
        if not isinstance(val, cqasm.v3.error_model.ErrorModelRef):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('model must be of type cqasm.v3.error_model.ErrorModelRef')
            val = cqasm.v3.error_model.ErrorModelRef(val)
        self._attr_model = val

    @model.deleter
    def model(self):
        self._attr_model = cqasm.v3.error_model.ErrorModelRef()
    ...
    def name(self, val):
        ...
        if not isinstance(val, cqasm.v3.primitives.Str):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('name must be of type cqasm.v3.primitives.Str')
            val = cqasm.v3.primitives.Str(val)
        self._attr_name = val

    @name.deleter
    def name(self):
        self._attr_name = cqasm.v3.primitives.Str()
    ...
    def parameters(self, val):
        ...
        if not isinstance(val, cqasm.v3.values.MultiNode):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('parameters must be of type cqasm.v3.values.MultiNode')
            val = cqasm.v3.values.MultiNode(val)
        self._attr_parameters = val

    @parameters.deleter
    def parameters(self):
        self._attr_parameters = cqasm.v3.values.MultiNode()
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if hasattr(cqasm.v3.error_model.ErrorModelRef, 'deserialize_cbor'):
            f_model = cqasm.v3.error_model.ErrorModelRef.deserialize_cbor(field)
        else:
            f_model = cqasm.v3.primitives.deserialize(cqasm.v3.error_model.ErrorModelRef, field)
        ...
        if hasattr(cqasm.v3.primitives.Str, 'deserialize_cbor'):
            f_name = cqasm.v3.primitives.Str.deserialize_cbor(field)
        else:
            f_name = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Str, field)
        ...
        if not isinstance(data, list):
            raise ValueError('missing serialization of Any/Many contents')
        f_parameters = cqasm.v3.values.MultiNode()
        for element in data:
            if element.get('@T') != '1':
                raise ValueError('unexpected edge type for Any/Many element')
            f_parameters.append(cqasm.v3.values.Node._deserialize(element, seq_to_ob, links))
    ...
    def _serialize(self, id_map):
        ...
        if hasattr(self._attr_model, 'serialize_cbor'):
            ...
            cbor['model'] = cqasm.v3.primitives.serialize(cqasm.v3.error_model.ErrorModelRef, self._attr_model)

        # Serialize the name field.
        if hasattr(self._attr_name, 'serialize_cbor'):
            cbor['name'] = self._attr_name.serialize_cbor()
        else:
            cbor['name'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Str, self._attr_name)
...
class ForLoop(Structured):
    ...
    def condition(self, val):
        if val is None:
            del self.condition
            return
        if not isinstance(val, cqasm.v3.values.Node):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('condition must be of type cqasm.v3.values.Node')
            val = cqasm.v3.values.Node(val)
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if field.get('@t', None) is None:
            ...
            f_condition = cqasm.v3.values.Node._deserialize(field, seq_to_ob, links)
...
class ForeachLoop(Structured):
    ...
    def lhs(self, val):
        ...
        if not isinstance(val, cqasm.v3.values.Node):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('lhs must be of type cqasm.v3.values.Node')
            val = cqasm.v3.values.Node(val)
        self._attr_lhs = val
    ...
    def frm(self, val):
        if val is None:
            del self.frm
            return
        if not isinstance(val, cqasm.v3.primitives.Int):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('frm must be of type cqasm.v3.primitives.Int')
            val = cqasm.v3.primitives.Int(val)
        self._attr_frm = val

    @frm.deleter
    def frm(self):
        self._attr_frm = cqasm.v3.primitives.Int()
    ...
    def to(self, val):
        ...
        if not isinstance(val, cqasm.v3.primitives.Int):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('to must be of type cqasm.v3.primitives.Int')
            val = cqasm.v3.primitives.Int(val)
        self._attr_to = val

    @to.deleter
    def to(self):
        self._attr_to = cqasm.v3.primitives.Int()
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if field.get('@t', None) is None:
            ...
            f_lhs = cqasm.v3.values.Node._deserialize(field, seq_to_ob, links)

        # Deserialize the frm field.
        field = cbor.get('frm', None)
        if not isinstance(field, dict):
            raise ValueError('missing or invalid serialization of field frm')
        if hasattr(cqasm.v3.primitives.Int, 'deserialize_cbor'):
            f_frm = cqasm.v3.primitives.Int.deserialize_cbor(field)
        else:
            f_frm = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Int, field)
        ...
        if hasattr(cqasm.v3.primitives.Int, 'deserialize_cbor'):
            f_to = cqasm.v3.primitives.Int.deserialize_cbor(field)
        else:
            f_to = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Int, field)
    ...
    def _serialize(self, id_map):
        ...
        if hasattr(self._attr_frm, 'serialize_cbor'):
            ...
            cbor['frm'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Int, self._attr_frm)

        # Serialize the to field.
        if hasattr(self._attr_to, 'serialize_cbor'):
            cbor['to'] = self._attr_to.serialize_cbor()
        else:
            cbor['to'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Int, self._attr_to)
...
class Instruction(InstructionBase):
    ...
    def instruction(self, val):
        if val is None:
            ...
            return
        if not isinstance(val, cqasm.v3.instruction.InstructionRef):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('instruction must be of type cqasm.v3.instruction.InstructionRef')
            val = cqasm.v3.instruction.InstructionRef(val)
        self._attr_instruction = val

    @instruction.deleter
    def instruction(self):
        self._attr_instruction = cqasm.v3.instruction.InstructionRef()
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if hasattr(cqasm.v3.instruction.InstructionRef, 'deserialize_cbor'):
            f_instruction = cqasm.v3.instruction.InstructionRef.deserialize_cbor(field)
        else:
            f_instruction = cqasm.v3.primitives.deserialize(cqasm.v3.instruction.InstructionRef, field)
    ...
    def _serialize(self, id_map):
        ...
        if hasattr(self._attr_instruction, 'serialize_cbor'):
            ...
            cbor['instruction'] = cqasm.v3.primitives.serialize(cqasm.v3.instruction.InstructionRef, self._attr_instruction)
...
class Mapping(Annotated):
    ...
    def value(self, val):
        ...
        if not isinstance(val, cqasm.v3.values.Node):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('value must be of type cqasm.v3.values.Node')
            val = cqasm.v3.values.Node(val)
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if field.get('@t', None) is None:
            ...
            f_value = cqasm.v3.values.Node._deserialize(field, seq_to_ob, links)
...
class Program(Node):
    ...
    def num_qubits(self, val):
        ...
        if not isinstance(val, cqasm.v3.primitives.Int):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('num_qubits must be of type cqasm.v3.primitives.Int')
            val = cqasm.v3.primitives.Int(val)
        self._attr_num_qubits = val

    @num_qubits.deleter
    def num_qubits(self):
        self._attr_num_qubits = cqasm.v3.primitives.Int()
    ...
    def api_version(self, val):
        if val is None:
            ...
            return
        if not isinstance(val, cqasm.v3.primitives.Version):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('api_version must be of type cqasm.v3.primitives.Version')
            val = cqasm.v3.primitives.Version(val)
        self._attr_api_version = val

    @api_version.deleter
    def api_version(self):
        self._attr_api_version = cqasm.v3.primitives.Version()
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if hasattr(cqasm.v3.primitives.Int, 'deserialize_cbor'):
            f_num_qubits = cqasm.v3.primitives.Int.deserialize_cbor(field)
        else:
            f_num_qubits = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Int, field)
        ...
        if hasattr(cqasm.v3.primitives.Version, 'deserialize_cbor'):
            f_api_version = cqasm.v3.primitives.Version.deserialize_cbor(field)
        else:
            f_api_version = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Version, field)
    ...
    def _serialize(self, id_map):
        ...
        if hasattr(self._attr_num_qubits, 'serialize_cbor'):
            ...
            cbor['num_qubits'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Int, self._attr_num_qubits)
        ...
        if hasattr(self._attr_api_version, 'serialize_cbor'):
            ...
            cbor['api_version'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Version, self._attr_api_version)
...
class SetInstruction(InstructionBase):
    ...
    def rhs(self, val):
        ...
        if not isinstance(val, cqasm.v3.values.Node):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('rhs must be of type cqasm.v3.values.Node')
            val = cqasm.v3.values.Node(val)
        self._attr_rhs = val
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if field.get('@t', None) is None:
            ...
            f_rhs = cqasm.v3.values.Node._deserialize(field, seq_to_ob, links)
...
class Subcircuit(Annotated):
    ...
    def iterations(self, val):
        ...
        if not isinstance(val, cqasm.v3.primitives.Int):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('iterations must be of type cqasm.v3.primitives.Int')
            val = cqasm.v3.primitives.Int(val)
        self._attr_iterations = val

    @iterations.deleter
    def iterations(self):
        self._attr_iterations = cqasm.v3.primitives.Int()
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if hasattr(cqasm.v3.primitives.Int, 'deserialize_cbor'):
            f_iterations = cqasm.v3.primitives.Int.deserialize_cbor(field)
        else:
            f_iterations = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Int, field)
    ...
    def _serialize(self, id_map):
        ...
        if hasattr(self._attr_iterations, 'serialize_cbor'):
            ...
            cbor['iterations'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Int, self._attr_iterations)
...
class Variable(Annotated):
    ...
    def typ(self, val):
        if val is None:
            del self.typ
            return
        if not isinstance(val, cqasm.v3.types.Node):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('typ must be of type cqasm.v3.types.Node')
            val = cqasm.v3.types.Node(val)
        self._attr_typ = val
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if field.get('@t', None) is None:
            ...
            f_typ = cqasm.v3.types.Node._deserialize(field, seq_to_ob, links)
...
class Version(Node):
    ...
    def items(self, val):
        ...
        if not isinstance(val, cqasm.v3.primitives.Version):
            # Try to "typecast" if this isn't an obvious mistake.
            if isinstance(val, Node):
                raise TypeError('items must be of type cqasm.v3.primitives.Version')
            val = cqasm.v3.primitives.Version(val)
        self._attr_items = val

    @items.deleter
    def items(self):
        self._attr_items = cqasm.v3.primitives.Version()
    ...
    def _deserialize(cbor, seq_to_ob, links):
        ...
        if not isinstance(field, dict):
            raise ValueError('missing or invalid serialization of field items')
        if hasattr(cqasm.v3.primitives.Version, 'deserialize_cbor'):
            f_items = cqasm.v3.primitives.Version.deserialize_cbor(field)
        else:
            f_items = cqasm.v3.primitives.deserialize(cqasm.v3.primitives.Version, field)
    ...
    def _serialize(self, id_map):
        ...
        if hasattr(self._attr_items, 'serialize_cbor'):
            ...
            cbor['items'] = cqasm.v3.primitives.serialize(cqasm.v3.primitives.Version, self._attr_items)

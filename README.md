...
To generate the documentation, run `doxygen` in the root of the project. It'll end up in `doc/doxygen`.

You need both a *new enough*(TM) version of Doxygen and graphviz. On Linux you can probably get away with just using the versions on your package manager. Ultimately, the documentation should be generated by CI and pushed somewhere publicly readable so the end user doesn't have to generate the docs themselves, but this hasn't been done yet.

...
libqasm should work with the environment described for [building OpenQL from sources on Windows](https://openql.readthedocs.io/en/latest/developer/build.html#windows-specific-instructions). In case that link breaks, here's a summary.
...
- `build`: when building the C++ library, the top level README file instructs you to build from here.<br/>
  On Linux, MacOS, and Windows using MinGW, most build artifacts are placed in the root of this directory.<br/>
  On Windows using MSVC, some of the artifacts are placed in its `Debug` subdirectory.
